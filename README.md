...
Vibin is a focused coding-agent CLI built with Bun and TypeScript. It supports OpenAI, OpenRouter, and the official Anthropic Messages API, streams responses, uses bounded project context, and asks before making changes.

## Quick start

Install and run Vibin without cloning the repository:

```powershell
bunx --bun vibin
```

Or install it once for use from any project directory:
...
For Windows, download and run `VibinSetup.exe` from the GitHub release. It downloads the real `vibin.exe`, installs it into your user-local PATH, and launches it. Your configuration remains in `.vibin` and is not uploaded.
