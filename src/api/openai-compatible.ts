...
export class OpenAICompatibleProvider implements Provider {
  constructor(private readonly options: { apiKey: string; baseUrl: string; model: string; appName?: string }) {}

  async *stream(messages: ChatMessage[], tools: ToolDefinition[], signal?: AbortSignal): AsyncGenerator<ChatStreamEvent> {
    ...
    try {
      response = await fetch(`${this.options.baseUrl.replace(/\/$/, "")}/chat/completions`, {
        ...
        body: JSON.stringify({ model: this.options.model, messages, tools, tool_choice: "auto", stream: true }),
      });
    } catch (error) {
