...
function asProfile(value: unknown): ProviderProfile | undefined {
  ...
  if (!["openai", "openrouter", "compatible", "codex"].includes(String(raw.provider)) || typeof raw.model !== "string" || !raw.model.trim() || typeof raw.baseUrl !== "string" || !raw.baseUrl.trim()) return undefined;
...
export async function loadConfig(cwd: string): Promise<VibinConfig> {
  const env = { ...(await loadDotEnv(cwd)), ...process.env }; const path = join(cwd, ".vibin", "config.json");
  let stored: Record<string, unknown> = {};
  if (existsSync(path)) { try { stored = JSON.parse(await readFile(path, "utf8")) as Record<string, unknown>; } catch { throw new VibinError("Could not parse .vibin/config.json.", "Delete or repair the file, then run Vibin again."); } }
...
export async function saveConfig(cwd: string, config: VibinConfig): Promise<void> {
  const directory = join(cwd, ".vibin"); await mkdir(directory, { recursive: true });
