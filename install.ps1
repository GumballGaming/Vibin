...
$installDir = Join-Path $env:LOCALAPPDATA "Vibin\bin"
$sourceExe = Join-Path $PSScriptRoot "dist\vibin.exe"
$targetExe = Join-Path $installDir "vibin.exe"

if (-not (Test-Path -LiteralPath $sourceExe)) {
  throw "dist\vibin.exe was not found. Build the release executable first."
}
