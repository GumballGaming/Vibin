...
test("rejects malformed OpenRouter API keys before sending a request", () => {
  expect(hasUsableApiKey({ provider: "openrouter", apiKey: "sk-or-", model: "meta-llama/llama-3.3-70b-instruct:free", baseUrl: "https://openrouter.ai/api/v1" })).toBeFalse();
  expect(hasUsableApiKey({ provider: "openrouter", apiKey: "sk-or-v1-test-key", model: "meta-llama/llama-3.3-70b-instruct:free", baseUrl: "https://openrouter.ai/api/v1" })).toBeTrue();
});
